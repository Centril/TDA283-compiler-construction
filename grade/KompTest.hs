...
testCompilation c good fs = do
    x <- doesFileExist c
    forM fs $ \t -> report0 =<<
        if x then testCompilationProg c good t
             else return $ defRep {repCmd = c} ?!
                           ("compiler " ++ c ++ " not found")
...
runJVM libPath classFile src inp outp = runInDirectory $ do
    setCurrentDirectory $ takeDirectory classFile
    runProg ("java -cp .:" ++ libPath ++ " " ++ takeBaseName classFile)
            src inp outp
...
runLLVM libPath bcFile src inp outp = runInDirectory $ do
    setCurrentDirectory $ takeDirectory bcFile
    killFile "a.out"
    system $ unwords ["llvm-ld", bcFile, libPath ++ "/runtime.bc"]
    runProg "./a.out" src inp outp
...
runx86 libPath oFile src inp outp = runInDirectory $ do
    setCurrentDirectory $ takeDirectory oFile
    system "rm a.out"
    system $ unwords ["gcc", "-m32", oFile, libPath ++ "/runtime.o"]
    runProg "./a.out" src inp outp
