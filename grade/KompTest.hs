...
linkLLVM :: FilePath -> FilePath -> IO ()
linkLLVM libPath bcFile = do
  ...
  where
    ...
    linkLegacyLLVM =
      void $ system ("llvm-ld " ++ bcFile ++ " " ++ libPath </> runtimeBitcode)

    -- ...otherwise use llvm-link + gcc
    linkModernLLVM = withTempFile "." "a.s" $ \file h -> void $ do
      hClose h
      system $ intercalate " | "
        [ "llvm-link " ++ bcFile ++ " " ++ libPath </> runtimeBitcode
        , "opt"
        , "llc > " ++ file
        ]
      system ("gcc -oa.out " ++ file ++ " 2>/dev/null")
